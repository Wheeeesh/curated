...
-- Public bucket: avatars are shown to every member, and a public URL means the
-- <img> needs no token and can be cached by the CDN.
...
-- Each member may only write the one object named after their own id, so
-- nobody can overwrite somebody else's picture.
